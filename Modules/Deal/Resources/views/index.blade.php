  ...
  <div class="table-responsive mb-3">
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          <td><a href="{{route("deal.show", $item->id)}}">От {{ date("d-m-Y, H:i" , strtotime($item->created_at)) }}</a></td>
          <td>{{$item->material->name}}</td>
          <td>{{$item->material->volume}}</td>
          <td><a href="{{route("seller.show", $item->Seller->first()->id)}}">{{$item->Seller->first()->name}}</a></td>
          <td>{{$item->seller_price}}</td>
          <td>{{$item->buyer_price}}</td>
          <td><a href="{{route("buyer.show", $item->Buyer->first()->id)}}">{{$item->Buyer->first()->name}}</a></td>
